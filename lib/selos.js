...
// ── Gate da Jornada (decisão nº 4/6 do plano) ────────────────────────────────
// Chave geral desligada até depois do Encontro Bússola (~fim de agosto/2026).
// Allowlist permite ao Filippe ver em produção antes de liberar pra base.
// Virar a chave = JORNADA_LIBERADA: true via PR de 1 linha.
export const JORNADA_LIBERADA = false;
export const JORNADA_ALLOWLIST = ['filippex@gmail.com', 'filippe@metodointento.com.br'];

export function jornadaVisivel(email, tipoAluno, statusApp) {
  if (tipoAluno !== 'ENEM') return false; // escopo: mentoria/ENEM (decisão nº 5)
  if (statusApp && STATUS_FORA_DO_APP.includes(statusApp)) return false; // '' = usa
  if (JORNADA_LIBERADA) return true;
  return JORNADA_ALLOWLIST.includes(String(email || '').trim().toLowerCase());
}
