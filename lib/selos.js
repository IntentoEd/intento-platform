...
// Metal por POSIÇÃO absoluta do tier (decisão 02/09/2026): escadas de 3
// degraus terminam no ouro, as de 4 na platina — mesmo índice, mesmo metal
// em todos os selos. Só identidade visual: critérios e rótulos intocados.
const TIERS_METAIS = ['bronze', 'prata', 'ouro', 'platina', 'diamante'];
...
export const SELOS = [
  ...
  },
  {
    ...
    tiers: [
      { label: 'Primeira palavra', criterio: 'um encontro cumprido — parcial conta, se nenhuma meta perdida' },
    ...
    calc({ diarios }) {
      ...
      // CALIBRADO 04/09/2026 (gate §6.2, baselineCombinadoEncontros: S1=27%
      // de eventos cumpridos <30%): o T1 aceita cumprimento PARCIAL sem meta
      // perdida; T2/T4 seguem exigindo sequência de cumprimento cheio.
      const tk = mkTracker(3);
      ...
      for (let i = 0; i + 1 < lista.length; i++) {
        ...
        if (cumpriu || (parcial && naoBatidas === 0)) tk.bater(0, label);
        if (cumpriu) {
          seq++;
...
export function computarSelos({ registros, diarios, simulados, caderno, marcos }) {
  ...
  const todos = SELOS.map(s => {
    ...
    return {
      ...
      tierMetal: tierIdx >= 0 ? TIERS_METAIS[tierIdx] : null,
      ...
      proximo: proxTier ? {
        ...
        tierMetal: TIERS_METAIS[tierIdx + 1],
