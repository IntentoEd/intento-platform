...
//   Simulado  = espelho do Domínio sobre o RESULTADO FINAL da prova: média do
//   aproveitamento geral dos últimos 3 simulados concluídos (validade 10 sem);
//   <70 / 70–alvo / ≥alvo (alvo = nível-alvo do marco, padrão 85). ATIVA e
//   no elo desde 31/08/2026 (gate de data aposentado — ver bloco SIMULADO_*).
...
export function marcoCicloPendente({ marcos, diarios, hoje = new Date(), tipoAluno, statusApp }) {
  ...
  // (O one-off MARCO_ATIVO_A_PARTIR foi aposentado em 31/08/2026: o backfill
  // retroativo de C1/C2 cobriu a base — quem não tem marco é porque não viveu
  // o ciclo, e a guarda de vivência abaixo já segura esse caso.)
...
// ── Dimensão Simulado (Fase 2 — regra fechada 24/08/2026) ────────────────────
// Espelha o Domínio, mas sobre o RESULTADO FINAL da prova (aproveitamento
// GERAL do simulado — não o parcial por disciplina): média do aproveitamento
// dos últimos 3 simulados concluídos. Faixas <70 Aprendiz · 70–alvo Veterano ·
// ≥alvo Mestre, onde alvo = nível-alvo do marco mais recente (padrão 85).
// Validade de 10 semanas: sem simulado concluído recente → sem dado (cinza),
// NUNCA Aprendiz por ausência — ritmo é papel do selo Ensaio Geral e do
// /lider, não do carimbo. ENTRA no elo do Perfil. Ativação antecipada de
// 01/10 pra 31/08/2026 (decisão do Filippe): setembro vira o mês de correr
// atrás de simulado ANTES do retrato do C3 congelar — quem não tem simulado
// recente fica "sem dado" (cinza, não derruba), quem tem média <70 já aparece
// Aprendiz com um mês de reação antes do fechamento.
export const SIMULADO_JANELA_DIAS = 70; // 10 semanas
export const SIMULADO_ULTIMOS_N = 3;

// lista = sim.lista dos payloads (buscarDadosAluno / dadosPainel) — o campo
// `aproveitamento` já vem em % sobre o escopo (dia1/dia2/completo e Custom).
export function resumoSimulados(lista, hoje = new Date()) {
  const conc = (lista || [])
    .filter(s => String(s?.status || '') === 'Concluída')
    .map(s => ({ dt: parseDataPayload(s.data), aprov: Number(s.aproveitamento) }))
    .filter(s => s.dt && !isNaN(s.aprov))
    .sort((a, b) => a.dt - b.dt);
  if (!conc.length) return null;
  const ultimo = conc[conc.length - 1];
  if (hoje - ultimo.dt > SIMULADO_JANELA_DIAS * 86400000) return null; // stale → sem dado
  const ult = conc.slice(-SIMULADO_ULTIMOS_N);
  return {
    media: ult.reduce((s, x) => s + x.aprov, 0) / ult.length,
    n: ult.length,
    ultimoTs: ultimo.dt.getTime(),
  };
}

export function carimboSimulado(resumo, { nivelAlvo = NIVEL_ALVO_SIMULADO_PADRAO } = {}) {
  if (!resumo || resumo.media == null) return null; // sem simulado recente → sem dado
  return carimboPorFaixa(resumo.media, 70, nivelAlvo);
}

// Nível-alvo vigente = do marco mais recente que tenha um alvo VÁLIDO (71-100:
// as faixas são <70 / 70–alvo / ≥alvo — alvo ≤70 esvaziaria o Veterano e
// carimbaria Aprendiz quem está ACIMA do próprio alvo). Valor fora da faixa
// (inclusive em marcos já persistidos) cai no padrão 85.
export function nivelAlvoDosMarcos(marcos) {
  if (!Array.isArray(marcos)) return NIVEL_ALVO_SIMULADO_PADRAO;
  const ord = marcos
    .filter(m => m?.ciclo && ['C1', 'C2', 'C3', 'C4'].includes(m.ciclo))
    .sort((a, b) => (Number(a.ano) * 4 + ['C1', 'C2', 'C3', 'C4'].indexOf(a.ciclo))
                  - (Number(b.ano) * 4 + ['C1', 'C2', 'C3', 'C4'].indexOf(b.ciclo)));
  for (let i = ord.length - 1; i >= 0; i--) {
    const v = Number(ord[i].nivelAlvo);
    if (v > 70 && v <= 100) return v;
  }
  return NIVEL_ALVO_SIMULADO_PADRAO;
}
...
export function diagnosticoDimensional(a) {
  ...
  // Simulado (Fase 2): caller injeta mx.simuladoResumo (resumoSimulados) e
  // mx.nivelAlvoSimulado (nivelAlvoDosMarcos). Ausente/inativo → null (chip
  // cinza) e fora do elo — o /lider só entra quando o GAS expuser o resumo.
  const simulado = carimboSimulado(mx.simuladoResumo, { nivelAlvo: mx.nivelAlvoSimulado });
  // elo: dimensão menos avançada (prioridade Comportamento > Cobertura > Domínio > Simulado em empate)
  const dimsOrd = [['comportamento', comportamento], ['cobertura', cobertura], ['dominio', dominio], ['simulado', simulado]].filter(([, n]) => n);
  ...
  return {
    ...
    cobertura, dominio, simulado, perfil, eloDim, alerta, overstudying,
    alertaMotivo: motivosAlerta.join(' + ') || null,
    domMed, cobMed, aprov, revisoesAtrasadas: revAtrasadas,
    simMed: mx.simuladoResumo?.media ?? null, simN: mx.simuladoResumo?.n ?? 0,
  };
}
