...
//   <70 / 70–alvo / ≥alvo (alvo = nível-alvo do marco, padrão 85). Ativa pra
//   todos em 01/10/2026 e entra no elo (ver bloco SIMULADO_* abaixo).
...
// /lider, não do carimbo. Ativa pra TODOS em 01/10/2026 (fechamento do C3) e,
// quando ativa, ENTRA no elo do Perfil (decisões de 23-24/08/2026).
export const SIMULADO_ATIVO_A_PARTIR = new Date(2026, 9, 1);
...
export function carimboSimulado(resumo, { hoje = new Date(), nivelAlvo = NIVEL_ALVO_SIMULADO_PADRAO } = {}) {
  if (hoje < SIMULADO_ATIVO_A_PARTIR) return null; // inativo até o fechamento do C3
  if (!resumo || resumo.media == null) return null;
