...
//   Comportamento = pior(Presença, Aproveitamento) sobre janela móvel de 4
//                   semanas mensuráveis (ver bloco abaixo)
...
// agregação do dashboardLider (gas/Code.gs): últimas 4 semanas → matérias,
// últimas 12 → histórico, normPct (≤1 → ×100, ≤0 → descartado), linha dupla na
// mesma semana → dias pelo maior valor. Divergência aqui = carimbo diferente
...
const normPct = (v) => { const n = numBR(v); if (n == null || n <= 0) return null; return n <= 1 ? n * 100 : n; };
...
export function registrosParaMetricas(registros) {
  ...
  const materias = {};
  const acc = (val, kSoma, kCount) => {
    const n = normPct(val);
    if (n != null) { materias[kSoma] = (materias[kSoma] || 0) + n; materias[kCount] = (materias[kCount] || 0) + 1; }
  };
  filtrados.slice(-4).forEach(r => {
    acc(r[C.DOM_BIO], 'domBio', 'cDomBio'); acc(r[C.PROG_BIO], 'progBio', 'cProgBio');
    acc(r[C.DOM_QUI], 'domQui', 'cDomQui'); acc(r[C.PROG_QUI], 'progQui', 'cProgQui');
    acc(r[C.DOM_FIS], 'domFis', 'cDomFis'); acc(r[C.PROG_FIS], 'progFis', 'cProgFis');
    acc(r[C.DOM_MAT], 'domMat', 'cDomMat'); acc(r[C.PROG_MAT], 'progMat', 'cProgMat');
  });
